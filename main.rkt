...
;(define (crack-cipher strategies key)
 ; (define cc (crack-cipher-h strategies key))
  ;(if (null? cc) (let ((etai (crack-cipher-h (list strat:trigrams) key))) (if (null? etai) (let ((trig (crack-cipher-h (list strat:etai) key))) (if (null? trig) (crack-cipher-h '() key) trig)) etai)) cc))
                       
(define (crack-cipher-h strategies key);; Returns list of encryption keys.
  (if (not key) (list #f)
  (if (null? strategies) (let* ((dc (if key (algo:dictionary-closure key) #f)) 
                                (dc-swe (if dc (algo:secret-word-enumeration dc) #f)))
                             (if dc-swe (begin (utils:show-key dc-swe) (list dc-swe)) (list dc-swe)))
  ;; make use of `utils:ciphertext` and `utils:cipher-word-list`
  ;; DISPLAY A KEY AS SOON AS YOU FIND IT USING (show-key key)
  (remove* (list #f) (append*(lc (crack-cipher-h (cdr strategies) (utils:add-substitution subk key)) : subk <- ((car strategies) key)))))))


;(define (crack-cipher strategies key)
 ; (remove-duplicates (append (crack-cipher-h '() key) (append* (lc (crack-cipher-h (list s) key) : s <- strategies)))))
  ;(remove-duplicates (append (crack-cipher-h '() key) (crack-cipher-h (list strat:trigrams) key) (crack-cipher-h (list strat:etai) key))))
(define (crack-cipher strategies key)
  (let ((trig (crack-cipher-h (list strat:trigrams) key)))
    (if (null? trig) (remove-duplicates (append (crack-cipher-h '() key) (crack-cipher-h (list strat:etai) key))) trig)))
